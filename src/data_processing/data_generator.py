...
def generate(client: openai.Client, attempt=0):
    ...
    try:
        """
        model will generate something like this:
        ...
        for generated_example in generated_json:
            try:
                if not validate_json_string(generated_example["fixed_json"]):
            ...
            result.append({
                "invalid_json": generated_example["invalid_json"],
                "fixed_json": generated_example["fixed_json"]
            })
...
if __name__ == "__main__":
    ...
    # eventually will write all this data to jsonl file
    dataset = []
    ...
    while keep_going:
        ...
        if not batch_found:
            ...
            print(f"Warning: Empty batch generated ({consecutive_failtures})/5")

            if consecutive_failures >= 5:
                print("Error: Stopping due to repeated generation failures.")
